...
function($scope, $http, $timeout, $interval, $location, $anchorScroll){
  ...
  $scope.tabName = ['會員卡', '發現'];
  $scope.countryCode = ['+853','+86'];
  ...
  $scope.activeMember = function () {
    $scope.profile = {};
    $scope.profile.countryCode = '+853';
    $scope.profile.birthday = $scope.today;
    console.log("active_form");
    $scope.pushView('active_form');
  };

  $scope.applyActive = function (profile) {
    $scope.tooltipsSytle = {display: 'none'};
    if (!profile.countryCode || !profile.phone || !profile.email || !profile.birthday) {
      console.log('form error');
      $scope.tooltipsSytle = {display: 'block'};
      return;
    }
    console.log(profile);
  }
  ...
  $scope.levelUpText = function (level) {
    if (level == 0) {
      return "";
    }
    var upRangeText;
    var upRequireText;
    if (level.upRange = 'forever') {
      upRangeText = '不限期';
    } else if (level.upRange = 'year') {
      upRangeText = '一年';
    }
    if (level.upRequire = 'credit') {
      upRequireText = '獲得積分';
    } else if (level.upRequire = 'redeemCredit') {
      upRequireText = '消費積分';
    }

    return "升級要求：會員在"+upRangeText+"內，"+upRequireText+"達 "+level.upCredit+"積分。"
  };

  $scope.validateText = function (validate) {
    var temp = new Date(validate);
    var now = new Date();
  ...
  $scope.getLevelUpCredit = function () {
    $scope.levelUpCredit = 0;
    console.log('getLevelUpCredit');
    $http.get('/oneMember/getLevelUpCredit?openId='+$scope.openId+'&ad='+ad).success(function(data, status, headers, config) {
      console.log(data);
      $scope.levelUpCredit = data.result;
    }).error(function(data, status, headers, config) {

    });
  }

  $scope.levelUpBarStyle = function (upCredit) {
    var percent = Math.floor($scope.levelUpCredit/upCredit * 100);
